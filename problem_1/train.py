...
from datasets import get_cifar10, transform_no_aug
from tqdm import tqdm


def get_transforms(use_randaug=False):
    """
    :param use_randaug: whether to use randaugment
    :return: train_transforms, test_transforms
    """
    if use_randaug:
        raise NotImplementedError
    else:
        train_transforms = transform_no_aug
    test_transforms = transform_no_aug
    return train_transforms, test_transforms
...
def train_model(
    ...
    model.to(device)
    train_transforms, test_transforms = get_transforms(use_randaug)
    train_set, val_set, test_set = get_cifar10(train_transforms, test_transforms)
    ...
    # Initialize a new wandb run and log experiment config parameters
    # code here
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for inputs, labels in train_loader:
                ...
                global_step += 1
                # save necessary metrics in a dictionary
                # code here
...
def get_args():
    ...
    # exp description
    parser.add_argument('--run_name', type=str, default='baseline', help='a brief description of the experiment')
    # dirs
    parser.add_argument('--save_dir', type=str, default='./checkpoints/',
                        help='save best checkpoint to this dir')
    # training config
    parser.add_argument('--epochs', type=int, default=30, help='training epochs')
    parser.add_argument('--batch_size', type=int, default=256, help='batch size; modify this to fit your GPU memory')
    parser.add_argument('--lr', type=float, default=1e-3, help='Learning rate')
    parser.add_argument('--use_randaug', action='store_true', help='use randaugment')
